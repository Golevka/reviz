    ...
    struct DFA_state *state, struct generic_list *visited)
{
    ...
    for ( ; i_trans < n_trans; i_trans++)
    {
        /* DFS */
        if (generic_list_add(
                visited, &state->trans[i_trans].to, __cmp_addr) != 0) {
            __DFA_traverse(state->trans[i_trans].to, visited);
        }
    }
}
...
void DFA_dispose(struct DFA_state *start)
{
    ...
    __DFA_traverse(start, &state_list);
...
static void __DFA_states_dump_graphviz(
    const struct DFA_state *state, struct generic_list *visited, FILE *fp)
{
    ...
    for ( ; i_trans < n_trans; i_trans++)
    {
        /* dump states, acceptable states are presented as double circles */
        ...
        /* dump the successor states in a recursive way */
        if (generic_list_add(
                visited, &state->trans[i_trans].to, __cmp_addr) != 0) {
            __DFA_states_dump_graphviz(state->trans[i_trans].to, visited, fp);
        }
    }
}
...
void DFA_dump_graphviz_code(const struct DFA_state *start_state, FILE *fp)
{
    ...
    __DFA_states_dump_graphviz(start_state, &visited_state, fp);
