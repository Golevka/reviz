...
void free_NFA_state(struct NFA_state *state) { free(state); }
...
int NFA_state_transition_num(const struct NFA_state *state) {
  ...
  if (state->transition[0].trans_type != NFATT_NONE)
    return 1;
  else
    return 0;
}
 ...
 * list */
static void __NFA_traverse(struct NFA_state *state,
...
void NFA_dispose(struct NFA *nfa) {
  ...
  /* free all states */
  for (cur = (struct NFA_state **)visited.p_dat; i_state < visited.length;
       i_state++, cur++) {
    free_NFA_state(*cur);
  }
