...
static void __create_dfa_state_entry(const struct generic_list *states,
  ...
  qsort(entry->nfa_states.p_dat, entry->nfa_states.length,
        entry->nfa_states.elem_size, __cmp_addr);
...
static void __NFA_state_epsilon_closure(const struct NFA_state *state,
  ...
  for (; i_trans < n_trans; i_trans++) {
    if (state->transition[i_trans].trans_type == NFATT_EPSILON) {
      ...
      if (generic_list_add(visited, &state->to[i_trans], __cmp_addr) != 0) {
...
static void __NFA_collect_transition_chars(struct generic_list *states,
                                           struct generic_list *trans_char) {
  ...
  for (; i_state < n_states; i_state++, s++) {
    ...
    for (i_trans = 0; i_trans < n_trans; i_trans++) {
      if ((*s)->transition[i_trans].trans_type == NFATT_CHARACTER) {
        generic_list_add(trans_char, &((*s)->transition[i_trans].trans_char),
                         __cmp_char);
      }
    }
  }
 ...
 * states */
static void __NFA_collect_target_states(struct generic_list *states, char c,
                                        struct generic_list *new_states) {
  struct NFA_state **s = (struct NFA_state **)states->p_dat;
...
static struct DFA_state *__get_DFA_state_address(
  ...
  addr = (struct __dfa_state_entry *)generic_list_find(
      dfa_state_entry_list, &entry, __cmp_dfa_state_entry);
...
static void __mark_acceptable_states(
  ...
  for (entry = (struct __dfa_state_entry *)dfa_state_entry_list->p_dat;
    ...
    if (generic_list_find(&entry->nfa_states, &terminator, __cmp_addr) !=
        NULL) {
...
static void __NFA_to_DFA_rec(struct generic_list *states,
                             struct generic_list *dfa_state_entry_list) {
  ...
  for (c = (char *)trans_char.p_dat; i_char < trans_char.length;
       i_char++, c++) {
    ...
    if (if_rec) __NFA_to_DFA_rec(&new_states, dfa_state_entry_list);
